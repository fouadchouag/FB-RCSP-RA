"""LOSO - ACMCSP. EA in loader + bandpass + SVM RBF C=4."""
import sys, os, csv
import numpy as np
...
from features.acmcsp import ACMCSPFeatureExtractor
...
for ts in range(len(subjects)):
    print(f"\nTesting {subjects[ts]}")
    tr=ids!=ts; te=ids==ts
    acmcsp=ACMCSPFeatureExtractor(n_components=6,alpha=0.5)
    acmcsp.fit_population(X_all[tr])
    Ftr=acmcsp.fit_transform(X_all[tr],y_all[tr]); Fte=acmcsp.transform(X_all[te])
